...
const Bank = ({ openModal, closeModal }: BankProps) => {
  ...
  >
    <form
      method="dialog"
      className="modal-box max-w-6xl bg-slate-100 p-0 overflow-hidden"
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-6 py-4">
        <h3 className="text-2xl font-semibold tracking-wide">
          🏦 Bank Smart Contract Dashboard
        </h3>
        <p className="text-sm opacity-90">
          Manage deposits, withdrawals and view on-chain activity
        </p>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        {(loading || deploying) && (
          <div className="alert alert-info shadow-sm">
            ...
            <span>
              {deploying ? 'Deploying contract...' : 'Processing transaction...'}
            </span>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
          <div className="flex flex-col md:flex-row gap-4 md:items-end">
            ...
            <button
              className={`btn btn-accent px-6 ${deploying ? 'loading' : ''}`}
          ...
          {appAddress && (
            <div className="mt-4 bg-slate-50 border rounded-xl p-3 text-xs break-all">
              <span className="font-medium text-slate-600">
                Application Address:
              </span>
              <div className="text-indigo-600 font-mono">
                {appAddress}
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200 space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm text-slate-600">Memo (optional)</label>
              <input
